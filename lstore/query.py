...
class Query:
    ...
    def select(self, search_key, search_key_index, projected_columns_index):
        ...
        #can cut //not needed since select will never be called on key that doesnt exist
        #if len(rids) == 0:
        #    return False
    ...
    def select_version(self, search_key, search_key_index, projected_columns_index, relative_version):
        ...
        #rid key map
        # get one RID
        # get RID of base record, 
            #  then access indirection and get tail record, get specified column data we want
        rids = self.table.index.locate(search_key_index, search_key)
        
        #select assumption means not needed
        #if len(rids) == 0:
        #    return False
