    ...
    def update_record(self, primary_key, columns):
        ...
        for i in range(len(columns)):
            if base_schema[i] == 0 and columns[i] != None: # check if this column has ever been updated before, and that we are trying to update it
                ...
                if new_indirection == None: # base has no updates yet
                    first_update[INDIRECTION_COLUMN] = baseRID # (1) base rid becomes tail's indirection value
                else: # base already had an update
                    first_update[INDIRECTION_COLUMN] = self.read(INDIRECTION_COLUMN, baseRID)  # (1) whatever was in the indirection column of base goes into first_updates indirection
                ...
                for j in range(self.total_columns):
                    ...
                    if not page_range.tail_pages[page_range.tail_page[last_tail_page]][j].has_capacity(len(first_update[j])): # len(values[i]) is future proofing  -DH
                ...
                for i in range(self.total_columns):
                    ...
                    # use the page offsets that were saved earlier 
                    self.page_directory[(i, first_update[RID_COLUMN])] = (page_range_index, last_tail_page, page_offsets[i])

        # ----------------------------------------------------------------

        # whatever value in base indirection should be valid to just copy over now, no need to check 
        values[INDIRECTION_COLUMN] = self.read(INDIRECTION_COLUMN, baseRID)

        # change base indirection to new rid
